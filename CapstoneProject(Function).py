...
Daftar_barang = {
    'alat tulis': [
        {'Nama Barang': 'Pensil', 'Stock': 20, 'Harga': 5000, 'Merek': 'Faber-castell', 'Tanggal Produksi' : '03/01/2023'},
        {'Nama Barang': 'Bolpen', 'Stock': 25, 'Harga': 10000, 'Merek': 'Snowman', 'Tanggal Produksi' : '19/12/2022'}
    ],
    'peralatan mandi': [
        {'Nama Barang': 'Sabun mandi', 'Stock': 15, 'Harga': 35000, 'Merek': 'lifebuoy', 'Tanggal Produksi' : '30/05/2022'},
        {'Nama Barang': 'Shampoo', 'Stock': 20, 'Harga': 25000, 'Merek': 'Head & shoulders', 'Tanggal Produksi' : '20/04/2019'}
    ]
}
...
def keseluruhan_table():
    ...
    for jenis_barang, item_barang in Daftar_barang.items():
        ...
        table = PrettyTable(['Nama Barang', 'Stock', 'Harga', 'Merek', 'Tanggal Produksi'])
        for barang in sorted_items:
            table.add_row([barang['Nama Barang'], barang['Stock'], barang['Harga'], barang['Merek'], barang['Tanggal Produksi']])
        print(table)
        print('')
...
def tampilkan_tabel_tertentu(barang):
    ...
    for item in sorted_items:
        table.add_row([item['Nama Barang'], item['Stock'], item['Harga'], item['Merek'], item['Tanggal Produksi']])
    print(table)
            ...
            if Menu2 =='1':
                while True:
                    ...
                    if barangapa in ['alat tulis', 'peralatan mandi']:
                        ...
                        tanggalmasuk = input('masukkan Tanggal Produksi dengan format (DD/MM/YYYY): ')
                        new_item = {'Nama Barang': nama_barang.capitalize(), 'Stock': stock, 'Harga': harga, 'Merek': merek, 'Tanggal Produksi': tanggalmasuk}
                        break
                    else:
                        message(2)
                        print('')
                            ...
                            while not namabarangtidakketemu:
                                ...
                                for item in Daftar_barang[ubahapa]:
                                    if item['Nama Barang'].lower() == barangapaan.lower():
                                        ...
                                        item['Tanggal Produksi']= input('masukkan Tanggal Produksi dengan format (DD/MM/YYYY): ')
                                        message(9)
                                        break
